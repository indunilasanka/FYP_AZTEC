            ...
            for (File file : files) {
                if(!file.isDirectory()) {
                   ...
                   // FileReader reader = new FileReader(file);
                    document.add(new TextField(FIELD_CONTENTS,Terms.getTerms(path), Field.Store.YES));
        ...
        try {
            for(File directory: directories) {
                ...
                for(int i=0; i< strs.length; i++){
                    builder.add(new Term(FIELD_CONTENTS, strs[i]), i);
                }
...
//                query = queryParser.parse(searchString);
                ...
                ScoreDoc[] hits = indexSearcher.search(pq, 1000).scoreDocs;
